...
import { listLeafAssetBuckets } from "../../panelAccounts/assetTreeBuckets";
...
export function AddStockAccountForm({ assetGroups }: Props) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const leafBuckets = useMemo(
    () => listLeafAssetBuckets(assetGroups).filter((b) => STOCK_PANEL_BUCKETS.has(b.slug)),
    [assetGroups]
  );
  const defaultBucket =
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: queryKeys.assetTree() }),
