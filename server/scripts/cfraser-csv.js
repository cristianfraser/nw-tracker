...
export function parseSheetMonthCell(raw) {
    const s = raw?.trim();
    if (!s)
        return null;
 ...
 */
export function numUsdDotDecimal(v) {
    if (v == null || v === "")
        return null;
    if (typeof v === "number")
        return Number.isFinite(v) ? v : null;
...
export function readSemicolonCsv(filePath) {
...
export function emitSignedMonthlyMovement(ins, accountId, amount, occurredOn, note, unitsDelta = null) {
