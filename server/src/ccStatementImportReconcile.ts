...
export function reconcileBillingMonthMovements(
  ...
  const parsed = sumParsedSectionsClp(facturadoRows);
  ...
  if (monto != null && monto > 0) {
    ...
    const ok = closeEnough(billed, monto, Math.max(TOL_CLP, Math.abs(monto) * 0.001));
    checks.push({
  ...
  if (compras != null && compras > 0 && parsed.parsed_operaciones > 0) {
    ...
    const ok = closeEnoughOperaciones(parsed.parsed_operaciones, compras, "clp");
    checks.push({
...
function headerFromCsvRecords(records: CcStatementCsvRecord[]): {
  monto_facturado: number | null;
  compras_cargos: number | null;
  source_pdf: string | null;
} {
  ...
  const montoRaw = parseInt10(String(first.statement_monto_facturado ?? ""));
  const comprasRaw = parseInt10(String(first.statement_compras_cargos ?? ""));
  return {
    monto_facturado: montoRaw > 0 ? montoRaw : null,
    compras_cargos: comprasRaw > 0 ? comprasRaw : null,
...
export function assertCcImportReconcilesOrThrow(
  ...
  for (const billingMonth of affectedBillingMonths(incoming)) {
    ...
    if (!header || ((header.monto_facturado ?? 0) <= 0 && (header.compras_cargos ?? 0) <= 0)) {
      continue;
    }
