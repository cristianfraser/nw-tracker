...
import type { CcInstallmentPurchaseComputed } from "../../types";
    ...
    <TableMobileCard title={title}>
      ...
      <TableMobileCardSection>
        ...
        {dueColumn === "next" ? (
          <TableMobileCardRow
            label="Próxima cuota"
            value={
              purchase.next_due_month
                ? `${purchase.next_due_month} (${formatYmEs(purchase.next_due_month)})`
                : "—"
            }
          />
        ) : null}
      ...
      {dueColumn === "none" && purchase.payment_statements && purchase.payment_statements.length > 0 ? (
        <TableMobileCardSection>
          ...
          {purchase.payment_statements.map((st, idx) => (
            <div key={`${purchase.purchase_id}:st:${idx}`} className={cn("mono", "muted", styles.purchaseMeta)}>
