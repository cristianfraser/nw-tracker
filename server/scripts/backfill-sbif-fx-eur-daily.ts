/**
 * Backfill `fx_daily` (dólar observado) and `eur_daily` (euro observado) from CMF SBIF API,
 * year-by-year from portfolio start through the current Chile calendar year.
 *
 * Replaces sparse month-end-only FX/EUR rows with one row per business day from SBIF.
 *
...
import { fetchDolarYear, fetchEuroYear } from "../src/sbifApi.js";
...
async function main(): Promise<void> {
  ...
  console.log(`Backfill SBIF FX/EUR from ${startYmd} (years ${startY}–${endY})${DRY ? " [dry-run]" : ""}…`);

  let fxTotal = 0;
  let eurTotal = 0;
  for (let y = startY; y <= endY; y++) {
    process.stderr.write(`USD ${y}… `);
    const usd = (await fetchDolarYear(y, apiKey)).filter((r) => r.date >= startYmd);
    fxTotal += upsertFxRows(usd, DRY);
    console.error(`${usd.length} rows`);

    process.stderr.write(`EUR ${y}… `);
    const eur = (await fetchEuroYear(y, apiKey)).filter((r) => r.date >= startYmd);
